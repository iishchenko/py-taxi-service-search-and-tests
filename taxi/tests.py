...
class ModelTests(TestCase):
    ...
    def test_creating_a_driver(self):
        user = Driver.objects.create_user(username='testuser', password='password123', first_name='John', last_name='Doe', license_number='ABC1234')
...
class FormTests(TestCase):
    ...
    def test_driver_creation_form_valid(self):
        form = DriverCreationForm(data={'username': 'testuser', 'password1': 'TEMU9Uzu26UNfrT', 'password2': 'TEMU9Uzu26UNfrT', 'first_name': 'John', 'last_name': 'Doe', 'license_number': 'ABC12345'})
        form.is_valid()
        print(form.errors)
        self.assertTrue(form.is_valid())

    def test_driver_creation_form_invalid_license_number(self):
        form = DriverCreationForm(data={'username': 'testuser', 'password1': 'password123', 'password2': 'password123', 'first_name': 'John', 'last_name': 'Doe', 'license_number': '12345'})
    ...
    def test_car_form_valid(self):
        ...
        form = CarForm(data={'model': 'Model S', 'manufacturer': manufacturer, 'drivers': [drivers.pk]})
        self.assertTrue(form.is_valid())

    def test_driver_license_update_form_valid(self):
        user = Driver.objects.create_user(username='testuser', password='password123')
        form = DriverLicenseUpdateForm(instance=user, data={'license_number': 'DEF56789'})
        self.assertTrue(form.is_valid())
...
class ViewTests(TestCase):
    ...
    def test_create_driver_view_redirects_to_login_when_not_logged_in(self):
        response = self.client.post(reverse('taxi:driver-create'), data={'username': 'testuser', 'password1': 'password123', 'password2': 'password123', 'first_name': 'John', 'last_name': 'Doe', 'license_number': 'ABC1234'})
    ...
    def test_create_car_view_redirects_to_login_when_not_logged_in(self):
        ...
        response = self.client.post(reverse('taxi:car-create'), data={'model': 'Model X', 'manufacturer': manufacturer.id, 'drivers': [drivers.pk]})
        self.assertEqual(response.status_code, 302)
    ...
    def test_create_car_view_creates_car_when_logged_in(self):
        ...
        self.client.login(username='testuser', password='password123')
        manufacturer = Manufacturer.objects.create(name='Tesla', country='USA')
        data = {'model': 'Model X', 'manufacturer': manufacturer.id, 'drivers': [driver.pk]}
